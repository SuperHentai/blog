 ...
 */
    function post_get_roc_datetime(){
        $post = get_post();
        $gmt_timedate = $post->post_date;
        $array1 = explode(" ",$gmt_timedate);
        $array2 = explode("-",$array1[0]);
        $roc_year = $array2[0]-1911;
        $str = sprintf("中華民國%s年%s月%s日 %s",$roc_year,$array2[1],$array2[2],$array1[1]);
        return $str;
    }
function materialwp_posted_on() {
    ...
    $time_string = post_get_roc_datetime();
